...
RpcClient::RpcClient(const std::string &configPath, const std::string &clientId)
    : configPath_(configPath), clientId_(clientId), threadManagerInitialized_(false) {
    // Initialize thread manager using C API
    if (thread_manager_init(&threadManager_, 10) != 0) {
...
void RpcClient::rpcClientThreadFunc() {
    try {
        ...
        // Set running flag immediately after thread starts
        running_.store(true);
...
bool RpcClient::start() {
    ...
    try {
        // Create and start the RPC client thread using ThreadManager C API
        if (thread_create(&threadManager_, [](void* arg) -> void* {
            RpcClient* client = static_cast<RpcClient*>(arg);
            client->rpcClientThreadFunc();
            return nullptr;
        }, this, &rpcThreadId_) != 0) {
            std::cerr << "[RPC] Failed to create thread" << std::endl;
            return false;
        }

        // Poll for the running flag with timeout
...
void RpcClient::stop() {
    ...
    try {
        ...
        if (rpcThreadId_ > 0) {
            thread_stop(&threadManager_, rpcThreadId_);
        }
...
void RpcClient::publishMessage(const std::string &topic,
