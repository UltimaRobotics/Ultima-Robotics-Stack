...
static void on_connect_callback(struct mosquitto *mosq, void *obj, int rc) {
    ...
    if (rc == MOSQ_ERR_SUCCESS) {
        ...
        // Auto-subscribe to configured subscription topics
        for (int i = 0; i < client->config.json_added_subs.count; i++) {
            if (client->config.json_added_subs.topics[i]) {
                int sub_result = mosquitto_subscribe(mosq, NULL, client->config.json_added_subs.topics[i], client->config.qos);
                if (sub_result == MOSQ_ERR_SUCCESS) {
                    LOG_INFO_SIMPLE("Auto-subscribed to topic: %s", client->config.json_added_subs.topics[i]);
                } else {
                    LOG_ERROR_SIMPLE("Failed to auto-subscribe to topic %s: %d", client->config.json_added_subs.topics[i], sub_result);
                    client->errors_count++;
                }
            }
        }
    ...
    while (ur_atomic_load(&client->heartbeat_running) && ur_atomic_load(&client->running)) {
        ...
        if (ur_atomic_load(&client->heartbeat_running) && 
            ...
            if (!payload) {
                ...
                cJSON_AddStringToObject(heartbeat_json, "client", 
                ...
                cJSON_AddBoolToObject(heartbeat_json, "ssl", client->config.use_tls);
                
                // Add connection statistics
                cJSON_AddNumberToObject(heartbeat_json, "messages_sent", client->messages_sent);
                cJSON_AddNumberToObject(heartbeat_json, "messages_received", client->messages_received);
                cJSON_AddNumberToObject(heartbeat_json, "uptime_seconds", time(NULL) - client->last_activity);
            ...
            if (payload) {
                ...
                // Actually publish the heartbeat message
                int result = mosquitto_publish(client->mosq, NULL, client->config.heartbeat.topic, 
                                             strlen(payload), payload, client->config.qos, false);
                if (result == MOSQ_ERR_SUCCESS) {
                    client->messages_sent++;
                } else {
                    client->errors_count++;
                    LOG_ERROR_SIMPLE("Failed to publish heartbeat: %d", result);
                }
