...
void validate_file_serving(const PathsConfig& paths_config) {
    ...
    // Optional files (not required for server startup)
    ...
    for (const auto& file : optional_files) {
        ...
        if (fs::exists(file_path)) {
            ...
            std::cout << "⚠️  Optional file missing: " << file << std::endl;
        }
    }
...
void setup_routes() {
    ...
    // Root route handler - serve index.html directly
    server->get("/", [auth_handler = auth_handler.get(), paths_config](const HttpRequest& request) {
        HttpResponse response;
        
        // Serve index.html as the main entry point
        std::string index_path = paths_config.frontend_root + "/index.html";
        
        // Check if index.html exists
        namespace fs = std::filesystem;
        if (fs::exists(index_path)) {
            response.set_file_content(index_path, "text/html");
            response.status_code = 200;
        } else {
            // Fallback to authentication-based redirection if index.html doesn't exist
            UserInfo user_info;
            if (auth_handler->authenticate_request(request, user_info)) {
                // User is authenticated - check if source.html exists, otherwise show error
                std::string source_path = paths_config.frontend_root + "/source.html";
                if (fs::exists(source_path)) {
                    response.status_code = 302;
                    response.headers["Location"] = "/source.html";
                } else {
                    response.set_error(404, "Source page not found");
                }
            } else {
    ...
    // Login page route
    server->get("/login-page.html", [paths_config](const HttpRequest& request) {
        HttpResponse response;
        
        // Serve login-page.html
        std::string login_path = paths_config.frontend_root + "/login-page.html";
        
        namespace fs = std::filesystem;
        if (fs::exists(login_path)) {
            response.set_file_content(login_path, "text/html");
            response.status_code = 200;
        } else {
            response.set_error(404, "Login page not found");
        }
...
🚀 Open http://)" << server_config.host << ":" << server_config.port << R"(/ in your browser
...
int main(int argc, char* argv[]) {
    ...
    std::vector<std::string> critical_files = {
        ...
        paths_config.frontend_root + "/index.html"
    };
