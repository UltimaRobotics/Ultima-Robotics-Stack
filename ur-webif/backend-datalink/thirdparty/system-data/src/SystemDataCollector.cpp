#include "../include/SystemDataCollector.h"
#include "../../controlled-log/include/controlled_log.h"
#include <thread>
#include <mutex>
#include <fstream>
#include <sstream>
...
bool SystemDataCollector::start(int poll_interval_seconds) {
    ...
    LOG_UTILITY_INFO("[SystemDataCollector] Started with " + std::to_string(poll_interval_seconds_) + "s interval");
    return true;
}
...
void SystemDataCollector::stop() {
    ...
    LOG_UTILITY_INFO("[SystemDataCollector] Stopped");
}
...
void SystemDataCollector::collectLoop() {
    ...
    while (running_.load()) {
        try {
            ...
            if (collection_count % collection_progress_log_interval_ == 1) {
                LOG_UTILITY_INFO("[SystemDataCollector] Collected metrics #" + std::to_string(collection_count) + 
                    " - CPU: " + std::to_string(current_metrics_.cpu.usage_percent) + "% " +
                    "Memory: " + std::to_string(current_metrics_.ram.usage_percent) + "% " +
                    "Disk: " + std::to_string(current_metrics_.swap.usage_percent) + "%");
            }
    ...
    LOG_UTILITY_INFO("[SystemDataCollector] Collection loop stopped after " + std::to_string(collection_count) + " collections");
}
...
void SystemDataCollector::logError(const std::string& message) const {
    LOG_UTILITY_ERROR("[SystemDataCollector] ERROR: " + message);
}
