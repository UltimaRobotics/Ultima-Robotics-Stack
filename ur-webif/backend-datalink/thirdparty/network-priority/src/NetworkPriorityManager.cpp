...
NetworkPriorityManager::NetworkPriorityManager(DatabaseManager* db_manager) 
    ...
    LOG_UTILITY_INFO("[CRITICAL] NetworkPriorityManager constructor called with db_manager: " + std::string(db_manager ? "VALID" : "NULL"));
    ...
    try {
        ...
        LOG_UTILITY_INFO("[CRITICAL] NetworkPriorityManager thread manager initialized successfully");
        
    } catch (const std::exception& e) {
        LOG_UTILITY_ERROR("[CRITICAL] NetworkPriorityManager constructor failed: " + std::string(e.what()));
        log("Failed to initialize Network Priority Manager: " + std::string(e.what()));
        throw;
    }
}
...
void NetworkPriorityManager::log(const std::string& message) const {
    LOG_UTILITY_INFO("[NetworkPriorityManager] " + message);
}
