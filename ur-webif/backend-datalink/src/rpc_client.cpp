...
void RpcClient::rpcClientThreadFunc() {
    try {
        ...
        logInfo("Custom message handler registered on thread context");
        ...
        running_.store(true);
        logInfo("RPC client thread started, waiting for connection...");
...
void RpcClient::staticMessageHandler(const char *topic, const char *payload,
    ...
    LOG_RPC_INFO("[" + self->clientId_ + "] Message received on topic: " + topicStr + 
                 " (payload size: " + std::to_string(payload_len) + " bytes)");
    ...
    if (self->messageHandler_) {
        ...
        LOG_RPC_WARN("[" + self->clientId_ + "] No message handler set for topic: " + topicStr);
    }
}
...
void RpcClient::logInfo(const std::string& message) const {
    LOG_RPC_INFO("[" + clientId_ + "] " + message);
}

void RpcClient::logError(const std::string& message) const {
    LOG_RPC_ERROR("[" + clientId_ + "] ERROR: " + message);
}
...
void RpcOperationProcessor::sendResponseStatic(const std::string& transactionId, bool success,
    ...
    try {
        ...
        LOG_RPC_ERROR("Failed to send response: " + std::string(e.what()));
    }
}
