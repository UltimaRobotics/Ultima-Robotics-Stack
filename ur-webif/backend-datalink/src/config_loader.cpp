...
void ConfigLoader::parseDatabaseConfig(const json& db_config) {
    ...
    if (db_config.is_array()) {
        // New array format with separate runtime and system databases
        for (const auto& db_entry : db_config) {
            if (db_entry.contains("runtime_db_path")) {
                if (!db_entry["runtime_db_path"].is_string()) {
                    throw ConfigException("database.runtime_db_path must be a string");
                }
                db_config_.runtime_db_path = db_entry["runtime_db_path"];
            }
            
            if (db_entry.contains("system_db_path")) {
                if (!db_entry["system_db_path"].is_string()) {
                    throw ConfigException("database.system_db_path must be a string");
                }
                db_config_.system_db_path = db_entry["system_db_path"];
            }
            
            if (db_entry.contains("enabled")) {
                if (!db_entry["enabled"].is_boolean()) {
                    throw ConfigException("database.enabled must be a boolean");
                }
                db_config_.enabled = db_entry["enabled"];
            }
            
            if (db_entry.contains("log_connections")) {
                if (!db_entry["log_connections"].is_boolean()) {
                    throw ConfigException("database.log_connections must be a boolean");
                }
                db_config_.log_connections = db_entry["log_connections"];
            }
            
            if (db_entry.contains("log_messages")) {
                if (!db_entry["log_messages"].is_boolean()) {
                    throw ConfigException("database.log_messages must be a boolean");
                }
                db_config_.log_messages = db_entry["log_messages"];
            }
        }
    } else {
        // Legacy object format - map to runtime_db_path for backward compatibility
        if (db_config.contains("path")) {
            if (!db_config["path"].is_string()) {
                throw ConfigException("database.path must be a string");
            }
            db_config_.runtime_db_path = db_config["path"];
        }
