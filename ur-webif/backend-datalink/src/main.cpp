...
#include "../include/managed_websocket_server.h"
#include "../include/database_manager.h"
#include "../thirdparty/system-data/include/SystemDataCollector.h"
#include "../thirdparty/network-priority/include/NetworkPriorityManager.h"
...
std::unique_ptr<LicenseSyncCronJob> g_licenseSyncCronJob;
...
using BackendDatalink::g_licenseSyncCronJob;
...
void handleNetworkPriorityRequest(const std::string& connection_id, const json& message) {
    ...
    try {
        ...
        LOG_WEBSOCKETS_ERROR("Error handling network priority request: " + std::string(e.what()));
...
void signalHandler(int signal) {
    ...
    if (g_licenseSyncCronJob) {
        g_licenseSyncCronJob->stop();
        std::cout << "License sync cron job stopped" << std::endl;
    }
...
void broadcastDashboardUpdate(const std::string& category, const json& data) {
    ...
    LOG_WEBSOCKETS_INFO("[BROADCAST] Sent dashboard update for category: " + category);
}
...
int main(int argc, char* argv[]) {
    std::cout << "=== Backend-Datalink Starting ===" << std::endl;
    ...
    try {
        ...
        if (config_file.good()) {
            ...
            ControlledLog::LoggingConfig loggingConfig = ControlledLog::LoggingConfig::fromJson(config_json);
            ControlledLog::Logger::setConfig(loggingConfig);
        } else {
            // Fallback to default configuration
            ControlledLog::LoggingConfig simpleConfig;
            simpleConfig.websockets_enabled = true;
            simpleConfig.database_enabled = true;
            simpleConfig.licence_cron_enabled = true;
            simpleConfig.rpc_enabled = true;
            simpleConfig.utility_enabled = true;
            ControlledLog::Logger::setConfig(simpleConfig);
        }
        
        LOG_WEBSOCKETS_INFO("Backend-datalink starting up...");
        
        const auto& ws_config = config_loader.getWebSocketConfig();
        
        LOG_WEBSOCKETS_INFO("Configuration loaded - Host: " + ws_config.host + 
                           ", Port: " + std::to_string(ws_config.port) + 
                           ", Max connections: " + std::to_string(ws_config.max_connections) +
        ...
        if (!g_database->initialize(config_loader.getDatabaseConfig())) {
            LOG_DATABASE_ERROR("Failed to initialize database");
            return 1;
        }
        LOG_DATABASE_INFO("Database initialized successfully");
        ...
        auto messageRouter = std::make_shared<BackendDatalink::MessageRouter>();
        ...
        if (!g_rpcClient->start()) {
            LOG_WEBSOCKETS_ERROR("Failed to start RPC client");
            return 1;
        }
        LOG_WEBSOCKETS_INFO("RPC client started successfully");
        
        // Initialize and start license sync cron job
        LOG_LICENSE_CRON_INFO("Initializing license sync cron job...");
        g_licenseSyncCronJob = std::make_unique<LicenseSyncCronJob>(rpc_config_path, true);
        
        // Register license cron handlers with message router
        messageRouter->registerHandler("clients/ur-licence-mann/heartbeat", 
            [licenseJob = g_licenseSyncCronJob.get()](const std::string& topic, const std::string& payload) {
                licenseJob->handleHeartbeatMessage(topic, payload);
            });
        
        messageRouter->registerHandler("direct_messaging/ur-licence-mann/responses",
            [licenseJob = g_licenseSyncCronJob.get()](const std::string& topic, const std::string& payload) {
        ...
        if (!g_licenseSyncCronJob->start()) {
            LOG_LICENSE_CRON_ERROR("Failed to start license sync cron job");
            return 1;
        }
        LOG_LICENSE_CRON_INFO("License sync cron job started successfully");
