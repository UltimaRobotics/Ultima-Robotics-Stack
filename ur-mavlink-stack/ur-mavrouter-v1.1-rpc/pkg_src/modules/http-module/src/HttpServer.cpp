            ...
            [controller, this](const HttpRequest& req) {
                ...
                if (!req.body.empty()) {
                    try {
                        ...
                        if (j.contains("devicePath")) {
                            devicePath = j["devicePath"].get<std::string>();
                            std::cout << "[HTTP] Extracted device path from request: " << devicePath << std::endl;
                        }
                        
                        if (j.contains("baudrate")) {
                            baudrate = j["baudrate"].get<int>();
                            std::cout << "[HTTP] Extracted baudrate from request: " << baudrate << std::endl;
                        }
                    } catch (const std::exception& e) {
                        std::cout << "[HTTP] Warning: Failed to parse request body: " << e.what() << std::endl;
                    }
                }
                ...
                if (!devicePath.empty()) {
                    ...
                    try {
                        ...
                        std::cout << "[HTTP] Error updating router configuration: " << e.what() << std::endl;
                    }
                }
