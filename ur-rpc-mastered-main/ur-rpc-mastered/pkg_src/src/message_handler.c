...
int message_handler_puback(mqtt_client_t *client, mqtt_broker_t *broker, uint16_t packet_id) {
    ...
    LOG_DEBUG("Received PUBACK from %s (packet_id=%u)", client->client_id, packet_id);

    // Remove pending outgoing message
    client_manager_remove_pending_message(client, packet_id, true);
    // Also remove from incoming to prevent reprocessing
