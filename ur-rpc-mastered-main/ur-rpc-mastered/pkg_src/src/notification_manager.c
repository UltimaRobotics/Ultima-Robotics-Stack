...
int notification_manager_send_notification(notification_manager_t* manager,
    ...
    // Prevent notification loops - don't send notifications about notification messages
    if (event_type == NOTIFICATION_CLIENT_PUBLISH || event_type == NOTIFICATION_TOPIC_PUBLISH) {
        return 0; // Skip publish event notifications to prevent loops
    }
