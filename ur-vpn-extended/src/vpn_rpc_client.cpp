...
VpnRpcClient::VpnRpcClient(const std::string& configPath, const std::string& clientId)
    : configPath_(configPath), clientId_(clientId) {
    // Initialize thread manager
    threadManager_ = std::make_unique<ThreadMgr::ThreadManager>(10);
}

VpnRpcClient::~VpnRpcClient() {
    if (running_.load()) {
        stop();
    }
}

void VpnRpcClient::staticMessageHandler(const char* topic, const char* payload,
                                        size_t payload_len, void* user_data) {
    VpnRpcClient* self = static_cast<VpnRpcClient*>(user_data);
...
void VpnRpcClient::rpcClientThreadFunc() {
    try {
...
bool VpnRpcClient::isRunning() const { 
    return running_.load(); 
}

void VpnRpcClient::setMessageHandler(
    std::function<void(const std::string&, const std::string&)> handler) {
    messageHandler_ = std::move(handler);
}

void VpnRpcClient::sendResponse(const std::string& topic,
