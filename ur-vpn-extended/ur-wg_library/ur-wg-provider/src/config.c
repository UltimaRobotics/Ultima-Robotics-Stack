...
#define key_match(key) (value = get_value(line, key "="))
	...
	if (ctx->is_device_section) {
		...
		} else if (key_match("Address") || key_match("DNS") || key_match("MTU") || 
		           key_match("Table") || key_match("PreUp") || key_match("PostUp") || 
		           key_match("PreDown") || key_match("PostDown") || key_match("SaveConfig")) {
			/* Silently ignore wg-quick specific fields */
			ret = true;
		} else
			goto error;
