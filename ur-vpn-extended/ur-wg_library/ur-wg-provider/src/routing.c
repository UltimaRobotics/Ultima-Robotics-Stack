...
struct wg_routing_ctx {
    char interface_name[32];
    wg_route_rule_t rules[MAX_RULES];
    size_t rule_count;
...
static void fire_event(wg_routing_ctx_t *ctx, wg_route_event_type_t event_type,
                      const wg_route_rule_t *rule, const char *error_msg) {
    if (ctx->callback) {
        ctx->callback(event_type, rule, error_msg, ctx->callback_user_data);
    }
}

static int find_rule_index(wg_routing_ctx_t *ctx, const char *rule_id) {
...
static int apply_route_rule(wg_routing_ctx_t *ctx, wg_route_rule_t *rule) {
    char cmd[512];
    char dest_str[64];
    ...
    switch (rule->type) {
        ...
        case WG_ROUTE_TYPE_CUSTOM_EXCLUDE:
    ...
    if (result == 0) {
        rule->state = WG_ROUTE_STATE_APPLIED;
        rule->applied_time = time(NULL);
        fire_event(ctx, WG_ROUTE_EVENT_ADDED, rule, NULL);
    } else {
        rule->state = WG_ROUTE_STATE_FAILED;
        fire_event(ctx, WG_ROUTE_EVENT_FAILED, rule, "Failed to apply route");
    }
    
    return result;
}

static int remove_route_rule(wg_routing_ctx_t *ctx, wg_route_rule_t *rule) {
    char cmd[512];
    char dest_str[64];
    ...
    if (result == 0) {
        rule->state = WG_ROUTE_STATE_REMOVED;
        fire_event(ctx, WG_ROUTE_EVENT_REMOVED, rule, NULL);
    }
...
wg_routing_ctx_t* wg_routing_init(const char *interface_name) {
    ...
    wg_routing_ctx_t *ctx = malloc(sizeof(wg_routing_ctx_t));
    if (!ctx) {
        return NULL;
    }
    
    memset(ctx, 0, sizeof(wg_routing_ctx_t));
...
void wg_routing_cleanup(wg_routing_ctx_t *ctx) {
    ...
    wg_routing_stop_monitoring(ctx);
...
int wg_routing_add_rule(wg_routing_ctx_t *ctx, const wg_route_rule_t *rule) {
    ...
    memcpy(&ctx->rules[ctx->rule_count], rule, sizeof(wg_route_rule_t));
    ctx->rules[ctx->rule_count].state = WG_ROUTE_STATE_PENDING;
    ctx->rule_count++;
...
int wg_routing_remove_rule(wg_routing_ctx_t *ctx, const char *rule_id) {
    ...
    if (ctx->rules[idx].state == WG_ROUTE_STATE_APPLIED) {
        remove_route_rule(ctx, &ctx->rules[idx]);
    }
    
    for (size_t i = idx; i < ctx->rule_count - 1; i++) {
        memcpy(&ctx->rules[i], &ctx->rules[i + 1], sizeof(wg_route_rule_t));
    }
    ctx->rule_count--;
...
int wg_routing_update_rule(wg_routing_ctx_t *ctx, 
    ...
    memcpy(&ctx->rules[idx], updated_rule, sizeof(wg_route_rule_t));
    ctx->rules[idx].state = WG_ROUTE_STATE_PENDING;
...
int wg_routing_get_rule(wg_routing_ctx_t *ctx, 
    ...
    memcpy(out_rule, &ctx->rules[idx], sizeof(wg_route_rule_t));
...
int wg_routing_get_all_rules(wg_routing_ctx_t *ctx,
    ...
    *out_rules = malloc(sizeof(wg_route_rule_t) * ctx->rule_count);
    if (!*out_rules) {
        pthread_mutex_unlock(&ctx->lock);
        return -1;
    }
    
    memcpy(*out_rules, ctx->rules, sizeof(wg_route_rule_t) * ctx->rule_count);
...
int wg_routing_apply_rules(wg_routing_ctx_t *ctx) {
    ...
    int applied = 0;
    for (size_t i = 0; i < ctx->rule_count; i++) {
        if (ctx->rules[i].enabled && ctx->rules[i].state != WG_ROUTE_STATE_APPLIED) {
...
int wg_routing_clear_routes(wg_routing_ctx_t *ctx) {
    ...
    int removed = 0;
    for (size_t i = 0; i < ctx->rule_count; i++) {
        if (ctx->rules[i].state == WG_ROUTE_STATE_APPLIED) {
    ...
    while (line != NULL) {
        ...
        if (sscanf(line, "%63s metric %u", dest, &metric) >= 1 ||
            ...
            if (inet_pton(AF_INET, dest, &rule.dest_addr.ipv4) == 1) {
                ...
                rule.type = WG_ROUTE_TYPE_AUTOMATIC;
                ...
                if (ctx->rule_count < MAX_RULES) {
                    if (find_rule_index(ctx, rule.id) < 0) {
                        memcpy(&ctx->rules[ctx->rule_count], &rule, sizeof(wg_route_rule_t));
                        ctx->rule_count++;
                        fire_event(ctx, WG_ROUTE_EVENT_DETECTED, &rule, NULL);
                        detected++;
                    }
                }
            }
...
static void* monitor_thread_func(void *arg) {
    wg_routing_ctx_t *ctx = (wg_routing_ctx_t*)arg;
    
    while (ctx->monitor_running) {
        wg_routing_detect_routes(ctx);
