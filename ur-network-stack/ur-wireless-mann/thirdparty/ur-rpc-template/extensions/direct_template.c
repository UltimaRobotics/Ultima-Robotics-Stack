...
static void* direct_client_thread_func(void* arg) {
    ...
    while (thread_ctx->running) {
        ...
        if (!thread_ctx->client) {
            ...
            direct_client_log_info("RPC client created");
        ...
        // Main operation loop
        if (thread_ctx->connected && ur_rpc_client_is_connected(thread_ctx->client)) {
...
void direct_client_set_message_handler(direct_client_thread_t* thread_ctx, direct_message_handler_t handler, void* user_data) {
    if (!thread_ctx || !thread_ctx->client) return;
    
    // Note: This would require extending the ur-rpc API to support custom user data
