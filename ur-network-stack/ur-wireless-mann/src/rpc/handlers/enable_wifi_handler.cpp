...
EnableWifiHandler::EnableWifiHandler(
...
RPCResponse EnableWifiHandler::handle(const RPCRequest& request) {
    ...
    try {
        auto threadId = thread_manager_->createThread([this, transactionId, effectiveInterface]() {
            performEnableWifiInThread(transactionId, effectiveInterface);
        });
        
        uint64_t workerNum = rpc_service_->getNextWorkerNumber();
        std::string attachmentId = "worker-enablewifi-" + std::to_string(workerNum) + "-" + transactionId;
        thread_manager_->registerThread(threadId, attachmentId);
        
        json acceptedData;
        acceptedData["message"] = "Enable WiFi operation started";
...
void EnableWifiHandler::performEnableWifiInThread(const std::string& transactionId, const std::string& interface) {
    try {
        ...
        auto rfkillResult = executor.executeShell("rfkill unblock wifi 2>&1", std::chrono::seconds(5));
        
        // Bring interface up
        auto upResult = executor.executeShell("ip link set " + interface + " up 2>&1", std::chrono::seconds(5));
        
        auto endTime = std::chrono::steady_clock::now();
        auto executionTime = std::chrono::duration_cast<std::chrono::milliseconds>(
            endTime - startTime).count();
        
        if (!upResult.isOk() || upResult.value().exit_code != 0) {
            auto response = createErrorResponse(
                transactionId,
                ErrorCodes::OPERATION_FAILED,
                "Failed to bring interface up: " + (upResult.isOk() ? upResult.value().stderr_output : upResult.error())
            );
        ...
        // Update config
        config_manager_->setWirelessEnabled(true);
        
        json data;
        data["message"] = "WiFi enabled successfully";
...
RPCAction EnableWifiHandler::getSupportedAction() const {
    return RPCAction::EnableWifi;
}
