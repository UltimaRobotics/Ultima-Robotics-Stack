...
ForkedScanStrategy::ForkedScanStrategy(std::shared_ptr<ProcessExecutor> executor)
    : executor_(executor ? executor : std::make_shared<ProcessExecutor>())
    , parser_(std::make_shared<IwScanParser>()) {}

Result<ScanResult, std::string> ForkedScanStrategy::execute(const WifiInterface& interface) {
...
void ForkedScanStrategy::cancel() {
    cancelled_ = true;
}
