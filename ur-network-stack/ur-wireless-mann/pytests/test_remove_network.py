...
#!/usr/bin/env python3
"""
Test suite for remove_network RPC functionality.
Tests the ability to remove saved network credentials.
"""
...
class RemoveNetworkTest:
    def __init__(self, broker_host='127.0.0.1', broker_port=1899):
        self.broker_host = broker_host
        self.broker_port = broker_port
        self.client = mqtt.Client(client_id=f"test_remove_network_{uuid.uuid4().hex[:8]}")
    ...
    def test_remove_network(self, ssid="TestNetwork"):
        ...
        if self.response_received.get('action') != 'remove_network':
        ...
        if 'error' in self.response_received and self.response_received['error']:
            print(f"⚠ Response contains error (may be expected if network doesn't exist): {self.response_received['error']}")
        else:
            print(f"✓ No error in response")
            
        if 'result' in self.response_received:
            result = self.response_received['result']
            print(f"✓ Remove operation result:")
...
def main():
    ...
    success = tester.test_remove_network(ssid)
