...
#!/usr/bin/env python3
"""
Test suite for get_system_state RPC functionality.
Tests the ability to get comprehensive system state information.
"""
...
class GetSystemStateTest:
    def __init__(self, broker_host='127.0.0.1', broker_port=1899):
        self.broker_host = broker_host
        self.broker_port = broker_port
        self.client = mqtt.Client(client_id=f"test_get_system_state_{uuid.uuid4().hex[:8]}")
    ...
    def test_get_system_state(self):
        ...
        if self.response_received.get('action') != 'get_system_state':
        ...
        if 'result' in self.response_received:
            ...
            print(f"  - WiFi Enabled: {result.get('wifi_enabled', 'N/A')}")
            print(f"  - Current Mode: {result.get('current_mode', 'N/A')}")
            print(f"  - Interfaces Count: {len(result.get('interfaces', []))}")
            
            if 'interfaces' in result:
                for iface in result['interfaces']:
                    print(f"    - {iface.get('name', 'N/A')}: {iface.get('status', 'N/A')}")
        else:
...
def main():
    print("="*60)
    print("GET SYSTEM STATE RPC TEST")
    print("="*60)
    print(f"Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    tester = GetSystemStateTest()
    ...
    success = tester.test_get_system_state()
