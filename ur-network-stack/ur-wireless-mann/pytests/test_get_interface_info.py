...
#!/usr/bin/env python3
"""
Test suite for get_interface_info RPC functionality.
Tests the ability to get detailed information about a specific wireless interface.
"""
...
class GetInterfaceInfoTest:
    def __init__(self, broker_host='127.0.0.1', broker_port=1899):
        self.broker_host = broker_host
        self.broker_port = broker_port
        self.client = mqtt.Client(client_id=f"test_get_interface_info_{uuid.uuid4().hex[:8]}")
    ...
    def test_get_interface_info(self, interface_name="wlan0"):
        """Test getting interface information"""
        transaction_id = str(uuid.uuid4())
        
        request = {
            "action": "get_interface_info",
        ...
        print(f"\n{'='*60}")
        print(f"TEST: Get Interface Information")
        print(f"{'='*60}")
        ...
        if self.response_received.get('action') != 'get_interface_info':
        ...
        if 'result' in self.response_received:
            ...
            print(f"✓ Interface info received:")
            print(f"  - Name: {result.get('name', 'N/A')}")
            print(f"  - Status: {result.get('status', 'N/A')}")
            print(f"  - MAC: {result.get('mac_address', 'N/A')}")
            print(f"  - Driver: {result.get('driver', 'N/A')}")
            if 'current_network' in result:
                print(f"  - Current Network: {result['current_network'].get('ssid', 'N/A')}")
        else:
...
def main():
    print("="*60)
    print("GET INTERFACE INFO RPC TEST")
    print("="*60)
    ...
    success = tester.test_get_interface_info(interface_name)
