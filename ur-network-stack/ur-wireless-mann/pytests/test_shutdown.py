...
#!/usr/bin/env python3
"""
Test suite for shutdown RPC functionality.
Tests the ability to gracefully shutdown the ur-wireless-tools service.
"""
...
class ShutdownTest:
    def __init__(self, broker_host='127.0.0.1', broker_port=1899):
        ...
        self.client = mqtt.Client(client_id=f"test_shutdown_{uuid.uuid4().hex[:8]}")
    ...
    def test_shutdown(self):
        ...
        print(f"\n{'='*60}")
        print(f"TEST: Graceful Shutdown")
        print(f"{'='*60}")
        print(f"⚠️  WARNING: This will shutdown the ur-wireless-tools service!")
        ...
        if self.response_received.get('action') != 'shutdown':
...
def main():
    print("="*60)
    print("SHUTDOWN RPC TEST")
    print("="*60)
    print(f"Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # Ask for confirmation
    print("\n⚠️  WARNING: This test will shutdown the ur-wireless-tools service!")
