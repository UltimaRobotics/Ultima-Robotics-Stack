...
#!/usr/bin/env python3
"""
Test suite for get_status RPC functionality.
Tests the ability to get system status and operation statistics.
"""
...
class GetStatusTest:
    def __init__(self, broker_host='127.0.0.1', broker_port=1899):
        ...
        self.client = mqtt.Client(client_id=f"test_get_status_{uuid.uuid4().hex[:8]}")
    ...
    def test_get_status(self):
        ...
        print(f"\n{'='*60}")
        print(f"TEST: Get System Status")
        print(f"{'='*60}")
        ...
        if self.response_received.get('action') != 'get_status':
        ...
        if 'result' in self.response_received:
            ...
            print(f"  - Uptime: {result.get('uptime_seconds', 'N/A')}s")
            print(f"  - Active Operations: {result.get('active_operations', 'N/A')}")
            print(f"  - Total Operations: {result.get('total_operations', 'N/A')}")
            print(f"  - Active Threads: {result.get('active_threads', 'N/A')}")
            print(f"  - MQTT Connected: {result.get('mqtt_connected', 'N/A')}")
            print(f"  - Broker: {result.get('broker_address', 'N/A')}")
        else:
...
def main():
    print("="*60)
    print("GET STATUS RPC TEST")
    print("="*60)
    print(f"Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    tester = GetStatusTest()
    ...
    success = tester.test_get_status()
