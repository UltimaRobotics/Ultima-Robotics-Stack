...
#!/usr/bin/env python3
"""
Test suite for list_interfaces RPC functionality.
Tests the ability to list all wireless interfaces.
"""
...
class ListInterfacesTest:
    def __init__(self, broker_host='127.0.0.1', broker_port=1899):
        self.broker_host = broker_host
        self.broker_port = broker_port
        self.client = mqtt.Client(client_id=f"test_list_interfaces_{uuid.uuid4().hex[:8]}")
    ...
    def test_list_interfaces(self):
        ...
        print(f"\n{'='*60}")
        print(f"TEST: List Wireless Interfaces")
        print(f"{'='*60}")
        ...
        if self.response_received.get('transaction_id') != transaction_id:
            ...
            print(f"  Expected: {transaction_id}")
            print(f"  Got: {self.response_received.get('transaction_id')}")
            success = False
        else:
            print(f"✓ Transaction ID matches")
            
        # Check action matches
        if self.response_received.get('action') != 'list_interfaces':
            print(f"✗ Action mismatch")
            print(f"  Expected: list_interfaces")
            print(f"  Got: {self.response_received.get('action')}")
            success = False
        else:
        ...
        if 'result' in self.response_received:
            ...
            if 'interfaces' in result:
                ...
                for iface in interfaces:
                    ...
                    print(f"  - {name}: {status} (MAC: {mac})")
            else:
                print(f"✗ No 'interfaces' field in result")
                success = False
        else:
...
def main():
    print("="*60)
    print("LIST INTERFACES RPC TEST")
    print("="*60)
    print(f"Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    tester = ListInterfacesTest()
    ...
    success = tester.test_list_interfaces()
