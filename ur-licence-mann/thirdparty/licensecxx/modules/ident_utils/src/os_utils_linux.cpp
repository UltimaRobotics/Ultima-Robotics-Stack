...
namespace lcxx::experimental::ident_utils::os {
    ...
    auto get_info() -> os_info
    {
        ...
        for (int i = 0; i < buffer_len && i < static_cast<int>(group_ids.size()); ++i) {
            oi.user_gids.push_back(static_cast<std::size_t>(group_ids[i]));
        }
        std::sort( oi.user_gids.begin(), oi.user_gids.end() );
